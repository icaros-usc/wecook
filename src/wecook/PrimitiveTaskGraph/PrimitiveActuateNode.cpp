...
//        waitForUser("Please do " + m_motionType + "...");
        ...
        auto envCollisionGroup = objMgr->createCollisionGroupExceptFoodAndToMoveObj(m_manipulatedObj,
                                                                                    collisionDetector);
        ...
        if (m_motionType == "cut") {
            for (int j = 0; j < 3; j++) {
                ...
                auto motion5 =
                        std::make_shared<LinearDeltaMotionNode>(bn,
                                                                ...
                                                                armSkeleton,
                                                                collisionFreeConstraint);
                motion5->plan(robot->m_adaPlan, robot->m_adaExec);
            }
        } else if (m_motionType == "stir") {
            Eigen::Vector3d delta_x(-0.03, 0., -0.);
            delta_x << +0.03, 0., 0.00;
            auto motion6 =
                    std::make_shared<LinearDeltaMotionNode>(bn,
                                                            delta_x,
                                                            ...
                                                            armSkeleton,
                                                            collisionFreeConstraint);
            motion6->plan(robot->m_adaPlan, robot->m_adaExec);
            for (int j = 0; j < 3; j++) {
                Eigen::Vector3d delta_x(-0.03, 0., -0.);
                auto motion5 =
                        std::make_shared<LinearDeltaMotionNode>(bn,
            ...
            auto motionunconnect = std::make_shared<ConnMotionNode>(ingredientNode,
                                                                    ...
                                                                    armSpace,
                                                                    armSkeleton);
            motionunconnect->plan(robot->m_adaPlan, robot->m_adaExec);
            ...
            auto motionConnect = std::make_shared<ConnMotionNode>(ingredientNode,
                                                                  ...
                                                                  armSpace,
                                                                  armSkeleton);
            motionConnect->plan(robot->m_adaPlan, robot->m_adaExec);
            // grab again
