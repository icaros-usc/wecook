...
void executeTraj(
    ...
    auto collision = robot.getSelfCollisionConstraint(robot.getArm()->getStateSpace(),
                                                      robot.getArm()->getMetaSkeleton());
...
//  waitForUser("Press key to move arm to goal");
    ...
    auto smoothTrajectory
            = robot.postProcessPath<aikido::planner::parabolic::ParabolicSmoother>(traj.get(), satisfied,
