...
void TSRMotionNode::computeIK(
    ...
    if (!configurations.empty()) {
        ...
        configurationRanker =
                std::make_shared<const aikido::distance::NominalConfigurationRanker>(m_stateSpace, m_skeleton,
                                                                                     startState);
...
aikido::trajectory::TrajectoryPtr TSRMotionNode::planToConfiguration(
    ...
    aikido::planner::ompl::ompl_dynamic_pointer_cast<::ompl::geometric::RRTConnect>(
            plannerOMPL->getOMPLPlanner())->setRange(
            0.1);
...
void TSRMotionNode::executeTrajectory(const aikido::trajectory::TrajectoryPtr &timedTrajectory,
    ...
    future = adaExec->executeTrajectory(timedTrajectory);
    try {
        future.get();
    }
    catch (const std::exception &e) {
...
void TSRMotionNode::plan(const std::shared_ptr<ada::Ada> &adaPlan,
                         const std::shared_ptr<ada::Ada> &adaExec,
                         Result *result) {
    validate();
    ...
    while (curr_conf < configurations.size()) {
        ...
        auto trajectory = planToConfiguration(adaPlan,
                                              m_stateSpace,
                                              m_skeleton,
                                              startState,
                                              ...
                                              &planningResult);
        if (trajectory) {
            auto timedTrajectory = smoothTrajectory(trajectory, adaPlan);
            executeTrajectory(timedTrajectory, adaPlan, adaExec, startState);
            if (result) {
